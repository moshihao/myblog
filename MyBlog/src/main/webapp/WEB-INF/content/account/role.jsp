					...
					<td>&nbsp;
						<security:authorize ifAnyGranted="ROLE_浏览角色">
							<security:authorize ifNotGranted="ROLE_修改角色">
								<a href="role!input.action?id=${uuid}">查看</a>&nbsp;
							</security:authorize>
						</security:authorize>

						<security:authorize ifAnyGranted="ROLE_修改角色">
							<a href="role!input.action?uuid=${uuid}" id="editLink-${name}">修改</a>&nbsp;
							<a href="role!delete.action?uuid=${uuid}" id="deleteLink-${name}">删除</a>
						</security:authorize>
					</td>
				</tr>
			</s:iterator>
