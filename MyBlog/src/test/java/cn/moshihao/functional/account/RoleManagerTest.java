package cn.moshihao.functional.account;
...
import cn.moshihao.data.AccountData;
import cn.moshihao.entity.account.Authority;
import cn.moshihao.entity.account.Role;
import cn.moshihao.functional.BaseFunctionalTestCase;
import cn.moshihao.functional.Gui;
import cn.moshihao.functional.Gui.RoleColumn;
...
public class RoleManagerTest extends BaseFunctionalTestCase {
	...
	public void createRole() {
		...
		for (Authority authority : role.getAuthorityList()) {
			driver.findElement(By.id("checkedAuthIds-" + authority.getUuid())).setSelected();
		}
	...
	public void editRole() {
		...
		for (Authority authority : testRole.getAuthorityList()) {
			SeleniumUtils.uncheck(driver.findElement(By.id("checkedAuthIds-" + authority.getUuid())));
		}
	...
	private void verifyRole(Role role) {
		...
		for (Authority authority : role.getAuthorityList()) {
			assertTrue(driver.findElement(By.id("checkedAuthIds-" + authority.getUuid())).isSelected());
		}
		...
		for (Authority authority : uncheckAuthList) {
			assertFalse(driver.findElement(By.id("checkedAuthIds-" + authority.getUuid())).isSelected());
		}
	}
